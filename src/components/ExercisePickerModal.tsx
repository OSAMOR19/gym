    ...
    return (
        <button
            ...
            className={`
                ...
                ${isActive
                    ? 'border-[var(--card-color)] bg-white/[0.06]'
                    : 'border-white/8 bg-white/[0.03] hover:border-white/20 hover:bg-white/[0.05]'}
            `}
            style={{ '--card-color': color } as React.CSSProperties}
        >
            {/* Preview */}
            <div className="relative h-44 md:h-52 bg-[#161616] flex items-center justify-center overflow-hidden">
                {gif ? (
                    <>
                        {!gifLoaded && <div className="absolute inset-0 bg-white/5 animate-pulse" />}
                        <img
                            src={gif}
                            alt={exercise.name}
                            loading="lazy"
                ...
                <span className="absolute bottom-1.5 left-1.5 flex items-center gap-1 text-[8px] font-bold tracking-wider uppercase text-white/70 bg-black/60 backdrop-blur-sm rounded-md px-1.5 py-0.5">
            ...
            {/* Info */}
            <div className="p-3">
                <p className="text-sm font-semibold text-white leading-tight mb-1">{exercise.name}</p>
                <p className="text-[11px] text-white/30 leading-snug line-clamp-2">{exercise.description}</p>
            </div>
        </button>
    );
}
...
export default function ExercisePickerModal({
    ...
    const exercises = getExercisesByLabel(tab);
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 pt-4 pb-3">
                    <div>
                        <h2 className="text-base font-bold text-white">Choose Exercise</h2>
                        <p className="text-[10px] text-white/25 mt-0.5">Swipe through the cards, tap one to select</p>
                    </div>
                    <button
                        onClick={onClose}
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    ...
                                    ${isTab ? '' : 'text-white/35 border-white/8 hover:text-white/60 hover:border-white/20'}
                                `}
                                style={isTab ? { backgroundColor: `${c}18`, color: c, borderColor: `${c}45` } : {}}
                            >
                                {label}
                                <span className="ml-1.5 opacity-50 text-[9px]">{getExercisesByLabel(label).length}</span>
                            </button>
                        );
                    })}
                </div>
                ...
                <div className="relative pb-5">
                    ...
                    <button
                        ...
                        className="hidden md:flex absolute left-1 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/70 border border-white/10 items-center justify-center text-white/50 hover:text-white transition-all cursor-pointer"
                    ...
                    <button
                        onClick={() => scrollByCards(1)}
                        className="hidden md:flex absolute right-1 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/70 border border-white/10 items-center justify-center text-white/50 hover:text-white transition-all cursor-pointer"
