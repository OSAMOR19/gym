        ...
        <>
            ...
            {!open && (
                <button
                    ...
                    className={`fixed z-40 right-4 bottom-[5.75rem] md:right-6 md:bottom-6 w-14 h-14 rounded-full bg-accent text-black flex items-center justify-center shadow-[0_4px_25px_rgba(var(--accent-glow),0.4)] hover:bg-accent-strong active:scale-95 transition-all cursor-pointer ${attention ? 'animate-fab-breathe' : ''}`}
                ...
                >
                    ...
                    {nudges.length > 0 && (
                        <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1 rounded-full bg-red-500 border-2 border-app text-white text-[10px] font-bold flex items-center justify-center">
                            {nudges.length}
                        </span>
                    )}
                </button>
            ...
            {/* ─── Panel ───────────────────────────────────────────────────── */}
            {open && (
                <div className="fixed z-50 inset-0 md:inset-auto md:right-6 md:bottom-6 md:w-[400px] md:h-[640px] md:max-h-[calc(100dvh-3rem)] bg-app md:border md:border-ink/10 md:rounded-2xl md:shadow-2xl flex flex-col h-[100dvh] md:h-[640px] animate-chat-in">
                    {/* Header */}
                    <div className="flex-none flex items-center justify-between px-4 py-3 border-b border-ink/5" style={{ paddingTop: 'max(0.75rem, env(safe-area-inset-top))' }}>
                    ...
                    {view === 'history' && (
                        <div className="flex-1 overflow-y-auto p-3 space-y-1.5" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
                            <button
                                onClick={newConversation}
                                className="w-full flex items-center gap-2 px-3 py-3 rounded-xl bg-accent/10 border border-accent/20 text-accent text-sm font-semibold hover:bg-accent/15 transition-all cursor-pointer"
                            >
                            ...
                            {historyLoading && conversations.length === 0 && (
                                <div className="space-y-1.5">
                                    {[0, 1, 2].map((i) => (
                                        <div key={i} className="animate-pulse rounded-xl border border-ink/5 px-3 py-3">
                                            <div className="h-3 w-2/3 bg-ink/[0.05] rounded" />
                                            <div className="h-2 w-16 bg-ink/[0.04] rounded mt-2" />
                                        </div>
                                    ))}
                                </div>
                            )}
                            {!historyLoading && conversations.length === 0 && (
                                <div className="flex flex-col items-center text-center pt-12 pb-8 px-6">
                                    ...
                                    <div className="relative w-16 h-16 rounded-2xl bg-ink/[0.03] border border-ink/10 flex items-center justify-center mb-4">
                                        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-ink/25" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                            <path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" />
                                            <line x1="9" y1="11.5" x2="15" y2="11.5" stroke="rgba(var(--accent-glow),0.5)" />
                                        </svg>
                                        <span className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-accent/15 border border-accent/30 flex items-center justify-center">
                                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-accent" strokeWidth="2.5" strokeLinecap="round">
                                                <line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" />
                                            </svg>
                                        </span>
                                    </div>
                                    <p className="text-sm font-semibold text-ink/60 mb-1">Nothing here yet</p>
                            ...
                            {conversations.map((c) => (
                                <div
                                    key={c.id}
                                    className={`flex items-center gap-2 rounded-xl border transition-all ${c.id === conversationId ? 'border-accent/30 bg-accent/5' : 'border-ink/5 hover:border-ink/10'}`}
                                >
                                    <button
                                        onClick={() => openConversation(c.id)}
                                        className="flex-1 min-w-0 text-left px-3 py-3 cursor-pointer"
                                    >
                                        <p className="text-sm text-ink/70 truncate">{c.title}</p>
                                        <p className="text-[10px] text-ink/20 mt-0.5">{relativeDate(c.updated_at)}</p>
                                    </button>
                                    <button
                                        onClick={() => setPendingDelete(c)}
                                        aria-label={`Delete conversation: ${c.title}`}
                                        className="w-9 h-9 mr-1 flex items-center justify-center rounded-lg text-ink/15 hover:text-red-400 hover:bg-red-500/10 transition-all cursor-pointer flex-shrink-0"
                                    >
                        ...
                        <>
                            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                                {messages.length === 0 && !sending && (
                                    <div className="pt-6">
                                        <p className="text-sm text-ink/50 mb-1">Hey — I&apos;m your coach.</p>
                                        <p className="text-xs text-ink/25 mb-5">I can see your training history, plan, and recent sessions. Ask me anything.</p>
                                        <div className="flex flex-wrap gap-2">
                                            {chips.map((chip) => (
                                                <button
                                                    key={chip.key}
                                                    onClick={chip.onTap}
                                                    className="px-3 py-2 rounded-full border border-ink/10 text-xs text-ink/50 hover:border-accent/40 hover:text-accent transition-all cursor-pointer"
                                                >
                                                    {chip.label}
                                                </button>
                                            ))}
                                    ...
                                    <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <div
                                            className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap break-words ${m.role === 'user'
                                                ? 'bg-accent/15 text-ink/90 rounded-br-md'
                                                : m.error
                                ...
                                {sending && (
                                    <div className="flex justify-start">
                                        <div className="bg-ink/5 rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-1">
                                            {[0, 1, 2].map((d) => (
                                                <span
                                                    key={d}
                            ...
                            {/* Input */}
                            <div
                                className="flex-none border-t border-ink/5 p-3"
                            ...
                            >
                                <div className="flex items-end gap-2">
                                    <textarea
                                        ...
                                        className="flex-1 resize-none bg-ink/5 border border-ink/10 rounded-xl px-3.5 py-2.5 text-base md:text-sm text-ink/90 placeholder:text-ink/20 focus:outline-none focus:border-accent/40 max-h-28"
                                    />
                                    <button
                                        onClick={() => send()}
                                        disabled={!input.trim() || sending}
                                        aria-label="Send message"
                                        className="w-11 h-11 flex-shrink-0 rounded-xl bg-accent text-black flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed hover:bg-accent-strong transition-all cursor-pointer"
                                    >
                    ...
                    {pendingDelete && (
                        ...
                        >
                            ...
                            >
                                ...
                                <h3 id="delete-conv-title" className="text-sm font-bold text-ink mb-1.5">
                                    Delete this conversation?
                                </h3>
                                <p id="delete-conv-body" className="text-xs text-ink/40 leading-relaxed mb-5">
                                    <span className="text-ink/60">&ldquo;{pendingDelete.title}&rdquo;</span> and every message
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        disabled={deleting}
                                        className="flex-1 py-2.5 rounded-xl border border-ink/10 text-sm font-semibold text-ink/60 hover:bg-ink/5 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        ...
                                        disabled={deleting}
                                        className="flex-1 py-2.5 rounded-xl bg-red-500 text-sm font-bold text-ink hover:bg-red-600 transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                                    >
